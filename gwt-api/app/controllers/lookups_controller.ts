...
export default class LookupsController {
   ...
   */
    async deaths({ response }: HttpContext) {
      try {
        const [deathPlaces, deathSources, newbornDeathCauses] = await Promise.all([
          db.from('death_place').select('*').orderBy('death_place_id'),
          db.from('death_source').select('*').orderBy('death_source_id'),
          db.from('newborn_death_cuase').select('*').orderBy('newborn_death_cause_id'),
        ])
